    ...
    <section className="container-page section-pad">
      <div className="flex items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button asChild variant="ghost"><Link to="/products" onClick={freezeClick}>View all <ArrowRight /></Link></Button>
        </div>
      </div>
      <div
        ref={trackRef}
      ...
      >
        {categories.map((category, index) => (
          <div key={category.name} className="w-64 shrink-0 snap-start sm:w-72 [perspective:1400px]">
            <Link to={`/products?q=${encodeURIComponent(category.name)}`} onClick={freezeClick} className="group block h-80 w-full">
